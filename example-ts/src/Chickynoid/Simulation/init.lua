...
function Simulation.new(userId)
    ...
    self.state.pos = Vector3.new(0, 3, 0)
    ...
    self.constants.aimlock = 1 -- Lock Character to mouse

    self.constants.pushSpeed = 16 --set this lower than maxspeed if you want stuff to feel heavy
	self.constants.stepSize = 2.2
	self.constants.gravity = -198

    -- local WalkingMoveType = require(game.ReplicatedStorage.TS.Chicky.MoveStates.Walking)
    -- self:RegisterMoveState("Walking", WalkingMoveType.activeThink, nil, nil, nil)
...
function Simulation:SetPosition(position, teleport)
    ...
    self.characterData:SetTargetPosition(position, teleport)
end
